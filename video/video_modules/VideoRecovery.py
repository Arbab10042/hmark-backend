...
class VideoRecovery:
    ...
    def wrapper(self,itr,lookup,return_dict):
        return_dict[itr]=watermark_recovery(self.ArrayOfFrames[itr],lookup).imarr
    ...
    def VideoRecovery(self,lookup: np.ndarray=None,path: str=None):
        ...
        if __name__ == "__main__":
            ...
            self.RecoveredFrames=np.ones(self.ArrayOfFrames.shape,dtype=np.uint8)
            
            manager=mp.Manager()
            return_dict=manager.dict()
            processes=[]
            for i in range (0,len(self.ArrayOfFrames)):
                processes.append(mp.Process(target=self.wrapper,args=(i,lookup,return_dict)))
            print("appending")
            for i in range(0,len(self.ArrayOfFrames)):  
                processes[i].start()
            print("started")
            ...
            video = cv2.VideoWriter('recovered.mp4', fourcc, fps, (width, height))
            for i in range(len(self.RecoveredFrames)):
                video.write(self.RecoveredFrames[i])
            video.release()
            self.is_recovered=True
            
    def get_RecoveredFrames(self):
        return self.RecoveredFrames
